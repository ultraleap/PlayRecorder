...
namespace PlayRecorder.Leap
{
    public class LeapHandCache
    {
        ...
        public const int framePosition = 0, handStatCount = 8, leftHandOffset = 1, rightHandOffset = leftHandOffset + handStatCount + 1;
        ...
        public void UpdateStats()
        {
            ...
            if (grabAngle != hand.GrabStrength)
            {
                grabAngle = hand.GrabAngle;
                grabAngleUpdated = true;
            }
        ...
        public void RecordStatFrames(RecordItem recordItem, int currentTick, int offset)
        {
            ...
            if (grabAngleUpdated)
            {
                recordItem.parts[offset + 6].AddFrame(new LeapStatFrame(currentTick, grabAngle));
                grabAngleUpdated = false;
            }

            if (palmVelocityUpdated)
            {
                recordItem.parts[offset + 7].AddFrame(new LeapVectorStatFrame(currentTick, palmVelocity));
        ...
        public bool PlayHandStat(RecordFrame frame, int stat)
        {
            ...
            switch (stat)
            {
                ...
                case 5:
                    ...
                    return true;
                case 6:
                    grabAngle = ((LeapStatFrame)frame).stat;
                    return true;
                case 7:
